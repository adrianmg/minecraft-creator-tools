  ...
  private async _handleIntegrateItemOK() {
    ...
    if (this.state.dialogData) {
      try {
        ...
        // processItemSeed reads the picked File via text()/arrayBuffer(), which
        // can reject for a stale/unreadable File. This handler is wired to a
        // dialog button, so an unguarded rejection escapes as an
        // unhandledrejection. Log instead of crashing the promise chain.
        Log.error("Failed to integrate item from file: " + (e instanceof Error ? e.message : String(e)));
      }
    }
  }
  ...
  private async _processIncomingFile(path: string, file: File) {
    if (file != null && this.props.project != null && this.props.project.projectFolder != null) {
      try {
        ...
        // Reading a dropped File can reject if its on-disk snapshot changed or
        // became unreadable since the drop (e.g. the file moved, or came from a
        // transient content URI). This runs fire-and-forget from the drag-drop
        // entry.file() callback, so without this guard the rejection would
        // escape as an unhandledrejection.
        Log.error(
          "Failed to integrate dropped file '" + file.name + "': " + (e instanceof Error ? e.message : String(e))
        );
      }
